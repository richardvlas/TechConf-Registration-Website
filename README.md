...
This section provides an explanation and reasoning for the architecture selection of both the Azure Web App and Azure Function. Spliting the application into **Web App** and **Azure Functions** running all background jobs ensures modularity and has the advantage of scalability when the load is at peak. 
...
- Peformance: Using **Service Bus Queues** with **Azure Functions** as the main orchestrators of the background jobs, ensures that when the admin sends out  a notification, it is queued in the **Azure Service Bus Queue** and the **Azure function** is triggered when a new queue message is received. This way the application doesn't crashes due to HTTP timeout exceptions at peak load and therefore ensures high performance of the application running on cloud.
- Cost: The new cloud architecture of the migrated application ensures cost-effectivness as well, since the services scale during high volume traffic and reduce during low volume traffic on the web app, which directly relates to the cost to be paid for each service. For instance, the **App Service Plan** allows to scale up/down between different tiers ranging from Dev/Test with F1 free tier up to Production tier intended for most production loads. The architecture uses another cost-efective service for serverless computing, the Azure Functions which allows to run code without provisioning VMs. The cost is rather neglectible in comparison to other services used. from the above cost analysis, one can note that 2 milions executions at rate of 2000ms/execution costs only $1.80. The biggest cost in the whole architecture is the **Azure Storage Account** with cost of $51.41. The cost is high due to the fact that I have selected redundancy option in a secondary region, which drives the cost higher but ensures that for applications requiring high durability, you can choose to additionally copy the data in your storage account to a secondary region that is hundreds of miles away from the primary region. For read access to the secondary region, I configured the storage account to use read-access geo-redundant storage (RA-GRS). This option is not necesary and without it the cost of the Azure Storage Account would be $20.73 which is significantly lower. The Azure Postgres Database also constitutes a higher cost, eventhough it is the minumal configuration one can select, namely Standard_B1ms (1 vCore) Compute Sku with storage of 32 GiB. Again scaling up/down the database is possible and easy in terms of Compute size as well as Storage size which can be both configured in the `Setting` under `Compute + Target`.
